    ...
    <section class="px-6 py-8">
        <main class="max-w-lg mx-auto mt-10 p-6 bg-gray-100 border border-gray-300 rounded">
            ...
            <form action="/register" method="post">
                ...
                <input type="text" name="name" id="name" class="border border-gray-400 p-2 w-full rounded" value="{{old('name')}}" />
                @error('name')
                <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                @enderror

                <label for="username" class="block mb-2 font-bold text-gray-700 mt-4">Username</label>
                <input type="text" name="username" id="username" class="border border-gray-400 p-2 w-full rounded" value="{{old('username')}}" />
                @error('username')
                <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                @enderror
